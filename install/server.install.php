...
$config = require 'install/settings.php';

chdir(getcwd() . '/..');

if (isset($config['server_os']['override_with_file']) && isset($config['server_os']['override_with_file']) !== null) {
    copy($config['server_os']['override_with_file'], $config['server_os']['name'] . '.jar');
} else {
    ...
    switch ($config['server_os']['type']) {
        case 'paper1.16.4':
            ...
            break;
        case 'waterfall':
            copy(sprintf('https://papermc.io/api/v1/waterfall/1.16/%s/download', $config['server_os']['build']), getcwd() . '/' . $config['server_os']['name'] . '.jar');
            break;
    }
